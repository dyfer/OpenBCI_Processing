...
int channelToRead = 7;//0-based; define which channel we use
float threshold = 3.5; //values above this will be treated as PDR is present... very rudimentary thresholding, more signal processing would be desired.
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if (theOscMessage.checkAddrPattern("/avgFilt")==true) {
    //process raw data
    float channel = theOscMessage.get(channelToRead).floatValue(); //0-based
    println("channel "+channelToRead+" average: "+channel);
