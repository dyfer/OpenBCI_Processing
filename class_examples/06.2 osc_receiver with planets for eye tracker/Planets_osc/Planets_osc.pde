...
float threshold = 0.3; //for decibel values 
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if (theOscMessage.checkAddrPattern("/pupil/norm_pos")==true) {
    //process amp data
    float x = theOscMessage.get(0).floatValue(); //0-based
    float y = theOscMessage.get(1).floatValue(); //0-based
    println("pupil position (x, y): "+x+", " +y);
    if (x>threshold) {
      scaler = scalerHi;
    } else {
      scaler = scalerLo;
  ...
  if (theOscMessage.checkAddrPattern("/pupil/diameter")==true) {
    ...
    println("pupil diameter : "+diameter);
