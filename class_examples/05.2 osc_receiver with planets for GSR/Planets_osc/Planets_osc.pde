...
float threshold = 0.1; //for microSiemens values 
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if (theOscMessage.checkAddrPattern("/microSiemens")==true) {
    //process microSiemens data
    float microSiemens = theOscMessage.get(channelToRead).floatValue(); //0-based
    println("microSiemens value is: "+microSiemens);
    if (microSiemens>threshold) {
      scaler = scalerHi;
    } else {
      scaler = scalerLo;
    }
